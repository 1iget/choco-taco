...
$extractDir = "$tempDir/$packageName-$version-win32"
...
if(-not (Test-Path "$targetDir/data")) {
    mkdir "$targetDir/data" | Write-Debug
}

$files = gci "$targetDir/data"
$mysqld = "$targetDir/bin/mysqld.exe"

if($files.Length -eq 0) {
    & $mysqld --defaults-file="$targetDir/my-default.ini" --initialize-insecure --console 
}